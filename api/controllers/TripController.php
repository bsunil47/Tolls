...
//                    $unregistered->attributes = ['registration_no'zxbj k => $params['registration_no'], 'vechical_type_id' => $params['vechical_type_id'], 'user_id' => $params['user_id'], 'created_on' => date("Y-m-d H:i:s"), 'vechical_id' => 'UR_' . $reg_no . '_' .$params['vechical_type_id'].'_' . $params['user_id']];
    ...
    public function actionGeofence()
    {
        ...
        if ($model) {
            ...
            $toll_name = Tolls::find()->where(['toll_id' => $model->toll_id])->one();
            ...
            if (!empty($params['us']) && $params['us'] == 1) {
                ...
                Payments::updateAll(['status_payed' => 1], ['trip_details_id' => $trip_details_id]);
                Trips::updateAll(['travel_date' => date("Y-m-d H:i:s")], ['trip_id' => $trip_details->trip_id]);
                $toll_cost = TollCosts::find()->where(['toll_id' => $model->toll_id, 'vechical_types_id' => $vechical_type->vechical_type_id])->one();
                $user = User::find()->where(['user_id'=> $user_id])->one();
                //$user = User::updateAll(['amount' => $user->amount - $toll_cost->single_trip_cost],['user_id'=> $user_id]);
                $user = User::find()->where(['user_id'=> $user_id])->one();
                $output = ['Code' => 200, 'Message' => 'Status Updated Sucessfully', 'Info' => array('Booth' => ($Booth) ? "Booth " . $Booth['booth_unique_id'] : 'Please look for InTag booth', 'toll_cost' => $toll_cost->single_trip_cost), 'amount' => $user->amount];
            } else {
...
width: 100%; margin-top: 3%; padding: 0 10%' align='center'>
                    <tbody>
                    <tr><td class='main-container' style='margin: 0 auto; padding:
...
box-shadow: 0px 0px 8px 0px rgba(0,0,0,0.1)' align='center' bgcolor='#FFFFFF'><table width='100%'><tbody><tr>
                    <td align='center'><div class='content-modules-template'><div class='content-modules-template--drop'><table style='width: 100%' class='print-hide'><tbody><tr><td align='center'><table style='border-collapse: collapse; width: 100%'><tbody><tr><td colspan='3' height='10' width='100%'></td></tr></tbody></table></td>
                    </tr></tbody></table><div class='module-wrapper'><table class='content-module' border='0' cellpadding='0' cellspacing='0' width='100%'>
     <tbody>
     <tr>
...
<img class='template-img template-logo' src='https://d14sm6a273ku3g.cloudfront.net/8fa54874eaf9c7df8a88d13f985d734b.png' style='height: auto' height='53' width='110'>
                </a>
      </td>
     </tr>
   </tbody></table>
...
width='100%'>
   <tbody><tr>
     <td colspan='3' align='center' valign='middle' width='80%'>
       <div class='template-h2 template-message font-montserrat' style='margin: 0; padding: 0; font-family: \"Helvetica Neue\", Helvetica,
...
<div class='module-wrapper receipt'>
   ...
   <table class='content-module' border='0' cellpadding='0' cellspacing='0' width='100%'>
     <tbody><tr>
       <td>
         <!-- node type 8 -->
...
letter-spacing: 0px!important; word-break: break-word'><span>$toll_name->toll_name
...
text-transform: uppercase; font-weight: 800!important; border-bottom:
3px solid #d3d3d3; word-break: break-word' height='30'><span>Total</span></td>
                       <td class='full-total' style='margin: 0; padding:
5px 0; font-family: Helvetica, Arial, sans-serif;
font-size: 11px; line-height: 13px; text-align: right; width: 20%;
color: #32373b; letter-spacing: 0; word-break: break-word' height='30'></td>
...
padding: 0; line-height: 16px; height: 15px' height='15' valign='middle'></td>
                   </tr>
                   <tr>
...
break-word' height='30' valign='middle'></td>
                   </tr>
                 </tfoot>
               </table>
             </td>
